...
def test_logger_base_format_message_edge_cases() -> None:
    ...
    assert (
        logger._format_message_with_args("test %q invalid", ("value",))
        == "test %q invalid value"
    )
    assert (
        logger._format_message_with_args("test %s %s", ("only_one",))
        == "test %s %s only_one"
    )
...
def test_trace_level_custom_logger_name() -> None:
    ...
    logger.trace(
        "Trace with %s and %d", "args", 42, _foundation_logger_name="custom.trace.args"
    )


def test_core_setup_environment_variable_edge_cases() -> None:
    with patch.dict(os.environ, {"FOUNDATION_LOG_LEVEL": "INVALID"}):
...
def test_level_filter_edge_cases() -> None:
    ...
    level_map: dict[LogLevelStr, int] = {
        "CRITICAL": 50,
        "ERROR": 40,
        "WARNING": 30,
        "INFO": 20,
...
def test_concurrent_setup_and_reset() -> None:
    ...
    threads = [
        threading.Thread(target=setup_worker if i % 2 == 0 else reset_worker)
        for i in range(4)
    ]
    for t in threads:
        t.start()
...
def test_complex_nested_logger_names() -> None:
    setup_telemetry()
    logger.get_logger("provide.foundation.core.sub.module.deep.nest").info(
        "Deep nested message"
    )
...
def test_exception_logging_edge_cases() -> None:
    ...
    try:
        ...
        logger.exception(
            "Connection error", domain="network", action="connect", status="error"
        )

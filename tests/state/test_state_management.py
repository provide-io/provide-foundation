...
class TestCircuitBreaker:
    ...
    def test_success_resets_failure_count(self, machine):
        machine.transition(CircuitBreakerEvent.FAILURE)
        machine.transition(CircuitBreakerEvent.SUCCESS)
        assert machine.current_state == "closed"
        assert machine.circuit_state.failure_count == 0

    def test_open_to_half_open_after_timeout(self, machine):
        machine.transition(CircuitBreakerEvent.FAILURE)
        machine.transition(CircuitBreakerEvent.FAILURE)
        assert machine.current_state == "open"
        time.sleep(0.15)
    ...
    def test_half_open_success_closes_circuit(self, machine):
        machine.transition(CircuitBreakerEvent.FAILURE)
        machine.transition(CircuitBreakerEvent.FAILURE)
        time.sleep(0.15)
        machine.transition(CircuitBreakerEvent.TIMEOUT)
    ...
    def test_half_open_failure_reopens_circuit(self, machine):
        machine.transition(CircuitBreakerEvent.FAILURE)
        machine.transition(CircuitBreakerEvent.FAILURE)
        time.sleep(0.15)
        machine.transition(CircuitBreakerEvent.TIMEOUT)
    ...
    def test_reset_event_from_open(self, machine):
        machine.transition(CircuitBreakerEvent.FAILURE)
        machine.transition(CircuitBreakerEvent.FAILURE)
        assert machine.current_state == "open"

...
from provide.foundation.process.runner import CompletedProcess, ProcessError, TimeoutError
...
class TestAsyncRunCommand:
    ...
    async def test_command_with_input(self):
        ...
        result = await async_run_command([
            sys.executable, "-c", "import sys; print(f'INPUT: {sys.stdin.read().strip()}')"
        ], input=b"test input")
    ...
    async def test_command_with_kwargs(self):
        ...
        result = await async_run_command(["echo", "test"], shell=False, some_kwarg="value")
    ...
    async def test_process_error_reraise(self):
        ...
        with patch('asyncio.create_subprocess_exec', side_effect=ProcessError("test error")):
...
class TestAsyncStreamCommand:
    ...
    async def test_stream_process_error_reraise(self):
        ...
        with patch('asyncio.create_subprocess_exec', side_effect=ProcessError("stream error")):
    ...
    async def test_stream_timeout_error_reraise(self):
        ...
        with patch('asyncio.create_subprocess_exec', side_effect=TimeoutError("stream timeout")):
...
class TestAsyncRunShell:
    ...
    async def test_shell_delegates_to_async_run_command(self):
        ...
        with patch('provide.foundation.process.async_runner.async_run_command') as mock_run:
    ...
    async def test_stream_readline_timeout(self):
        ...
        with patch('asyncio.create_subprocess_exec', return_value=mock_process):
            with patch('asyncio.wait_for', side_effect=asyncio.TimeoutError()):
                with pytest.raises(TimeoutError, match="Command timed out after"):
                    lines = []
                    async for line in async_stream_command(["echo", "test"], timeout=1.0):
                        lines.append(line)

...
class TestOpenObserveIntegration:
    ...
    def generate_bulk_logs(self, count: int = 1000) -> str:
        ...
        operations = ["handle_request", "process_payment", "authenticate_user", "send_email", "validate_token"]
    ...
    def test_bulk_ingestion_and_query(self):
        ...
        os.environ["OPENOBSERVE_USER"] = "someuserexample@provide.test"
        ...
        if response.hits:
            ...
            if sample_trace:
                ...
                print(f"✅ Found {len(trace_response.hits)} logs for trace {sample_trace}")
    ...
    def test_rapid_ingestion(self):
        ...
        for batch in range(10):
            ...
            print(f"   Batch {batch + 1}/10: {len(result.get('items', []))} logs in {result.get('took', 0)}ms")

...
from provide.foundation import logger, TelemetryConfig, setup_telemetry
...
class TestParsingCoverage:
    ...
    def test_auto_parse_with_string_type_hints(self) -> None:
        ...
        # This simulates how attrs might represent types when defined in scopes
        # where the type is not yet available, e.g., forward references.
        int_field = field(type='int')
        int_field.name = 'int_field'  # Manually set name for test clarity

        bool_field = field(type='bool')
        bool_field.name = 'bool_field'

        list_field = field(type='list')
        list_field.name = 'list_field'

        dict_field = field(type='dict')
        dict_field.name = 'dict_field'

        unknown_field = field(type='SomeUnknownType')
        unknown_field.name = 'unknown_field'

        assert auto_parse(int_field, "42") == 42
        assert auto_parse(bool_field, "true") is True
        assert auto_parse(list_field, "a,b") == ["a", "b"]
        assert auto_parse(dict_field, "k=v") == {"k": "v"}
        assert auto_parse(unknown_field, "some_string") == "some_string"

    def test_auto_parse_no_type_hint(self) -> None:
        """Test auto_parse for an attrs field with no type hint."""
        no_type_field = field()
        no_type_field.name = 'no_type_field'

...
def x_get_message_from_stdin__mutmut_7() -> tuple[str | None, int]:
    ...
    try:
        message = sys.stdin.read().strip()
        if not message:
            click.echo("Error: Empty input from stdin.", )
            return None, 1
        return message, 0
    except Exception as e:
        click.echo(f"Error reading from stdin: {e}", err=True)
...
def get_message_from_stdin(*args, **kwargs):
    result = _mutmut_trampoline(x_get_message_from_stdin__mutmut_orig, x_get_message_from_stdin__mutmut_mutants, args, kwargs)
    return result 
...
def _infer_and_parse_value(*args, **kwargs):
    result = _mutmut_trampoline(x__infer_and_parse_value__mutmut_orig, x__infer_and_parse_value__mutmut_mutants, args, kwargs)
    return result 
...
def x_build_attributes_from_args__mutmut_8(
    ...
    if json_attrs:
        try:
            json_dict = json_loads(json_attrs)
            if not isinstance(json_dict, dict):
                click.echo("Error: JSON attributes must be an object.", )
                return {}, 1
            attributes.update(json_dict)
        except (ValueError, TypeError, ValidationError) as e:
            click.echo(f"Error: Invalid JSON attributes: {e}", err=True)
...
def x_build_attributes_from_args__mutmut_18(
    ...
    if json_attrs:
        try:
            ...
            click.echo(f"Error: Invalid JSON attributes: {e}", )
            return {}, 1
...
def build_attributes_from_args(*args, **kwargs):
    result = _mutmut_trampoline(x_build_attributes_from_args__mutmut_orig, x_build_attributes_from_args__mutmut_mutants, args, kwargs)
    return result 
...
def x_parse_filter_string__mutmut_9(filter_str: str) -> dict[str, str]:
    ...
    try:
        return parse_dict(filter_str, item_separator=",", key_separator="=", )
    except ValueError as e:
        click.echo(f"Warning: Invalid filter format: {e}", err=True)
        return {}
...
def x_parse_filter_string__mutmut_16(filter_str: str) -> dict[str, str]:
    ...
    try:
        ...
        click.echo(f"Warning: Invalid filter format: {e}", )
        return {}
...
def parse_filter_string(*args, **kwargs):
    result = _mutmut_trampoline(x_parse_filter_string__mutmut_orig, x_parse_filter_string__mutmut_mutants, args, kwargs)
    return result 
...
def x_get_client_from_context__mutmut_9(ctx: Any) -> tuple[Any | None, int]:
    ...
    client = ctx.obj.get("client") if ctx.obj else None
    if not client:
        click.echo("Error: OpenObserve not configured.", )
        return None, 1
    return client, 0
...
def get_client_from_context(*args, **kwargs):
    result = _mutmut_trampoline(x_get_client_from_context__mutmut_orig, x_get_client_from_context__mutmut_mutants, args, kwargs)
    return result 

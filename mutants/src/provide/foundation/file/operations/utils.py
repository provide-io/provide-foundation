...
def x_detect_atomic_save__mutmut_6(events: list[FileEvent]) -> FileOperation | None:
    ...
    operations = detector.detect(events)
    return next(
        (op for op in operations if op.operation_type == OperationType.ATOMIC_SAVE),
    )
...
def detect_atomic_save(*args, **kwargs):
    result = _mutmut_trampoline(
        x_detect_atomic_save__mutmut_orig, x_detect_atomic_save__mutmut_mutants, args, kwargs
    )
    return result
...
x_is_temp_file__mutmut_mutants: ClassVar[MutantDict] = {"x_is_temp_file__mutmut_1": x_is_temp_file__mutmut_1}
...
def extract_original_path(*args, **kwargs):
    result = _mutmut_trampoline(
        x_extract_original_path__mutmut_orig, x_extract_original_path__mutmut_mutants, args, kwargs
    )
    return result
...
def x_group_related_events__mutmut_orig(
...
def x_group_related_events__mutmut_1(
    events: list[FileEvent], time_window_ms: int = 501
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_2(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_3(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_4(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_5(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_6(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_7(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_8(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_9(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def x_group_related_events__mutmut_10(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
    ...
    config = DetectorConfig(time_window_ms=time_window_ms)
    detector = OperationDetector(config)
    return detector._group_events_by_time(
        sorted(
            events,
        )
    )
...
def x_group_related_events__mutmut_11(
    events: list[FileEvent], time_window_ms: int = 500
) -> list[list[FileEvent]]:
    """Group events that occur within a time window."""
    from provide.foundation.file.operations.detectors.orchestrator import OperationDetector
...
def group_related_events(*args, **kwargs):
    result = _mutmut_trampoline(
        x_group_related_events__mutmut_orig, x_group_related_events__mutmut_mutants, args, kwargs
    )
    return result

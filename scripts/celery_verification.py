...
def verify_extreme_test_segment(self, test_type: str, duration_seconds: int = 10) -> dict[str, Any]:
    ...
    try:
        ...
        while time.time() < end_time:
            try:
                extreme_logger.info(
                    f"extreme_test_message_{message_count}",
                    test_type=test_type,
...
def verify_cut_up_chuck_segment(self, duration_seconds: int = 30) -> dict[str, Any]:
    ...
    try:
        ...
        result = {
            ...
            "task_rate": (entries_generated + batches_generated + anomalies_detected) / actual_duration
...
def collect_verification_results(self, results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    try:
        ...
        total_throughput = sum(
            r.get("throughput_per_second", 0) or r.get("messages_per_second", 0) or r.get("task_rate", 0)
            for r in results
        )
        ...
        if benchmark_results:
            benchmark_analysis = {
                ...
                "avg_throughput": sum(r.get("throughput_per_second", 0) for r in benchmark_results)
        ...
        if extreme_results:
            ...
            extreme_analysis = {
                ...
                "avg_message_rate": sum(r.get("messages_per_second", 0) for r in extreme_results)
                / len(extreme_results),

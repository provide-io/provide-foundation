...
class SimpleCounter:
    ...
    def __init__(self, name: str, otel_counter: Optional[Any] = None):
        self.name = name
...
class SimpleGauge:
    ...
    def __init__(self, name: str, otel_gauge: Optional[Any] = None):
        self.name = name
    ...
    def set(self, value: float, **labels) -> None:
        ...
        if self._otel_gauge:
            try:
                self._otel_gauge.add(value - self._labels_values.get(
                    ",".join(f"{k}={v}" for k, v in sorted(labels.items())) if labels else "", 0
                ), attributes=labels)
            except Exception as e:
...
class SimpleHistogram:
    ...
    def __init__(self, name: str, otel_histogram: Optional[Any] = None):
        self.name = name

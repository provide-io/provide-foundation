    ...
    if not quiet:
        ...
        for dep in deps:
            ...
            if not dep.available:
                print(f"     Install with: pip install 'provide-foundation[{dep.name}]'")
            print()
            
        print(f"📊 Summary: {available_count}/{total_count} optional dependencies available")
        
        if available_count == total_count:
            print("🎉 All optional features are available!")
        elif available_count == 0:
            print("💡 Install optional features with: pip install 'provide-foundation[all]'")
        else:

# env.ps1 - provide-foundation Development Environment Setup
#
...
if (-not (Test-Path "pyproject.toml")) {
    ...
    Write-Host "Please run this script from the provide-foundation root directory"
    exit 1
}
...
$Profile = if ($env:PROVIDE_FOUNDATION_PROFILE) { $env:PROVIDE_FOUNDATION_PROFILE } else { "default" }
if ($Profile -eq "default") {
    $VenvDir = "workenv/provide-foundation_${TFOS}_${TFARCH}"
} else {
...
$LogDir = Join-Path $env:TEMP "provide-foundation_setup"
...
Write-Host "Installing provide-foundation in editable mode..." -NoNewline
try {
    & uv pip install --no-deps -e . 2>&1 | Out-File -FilePath (Join-Path $LogDir "install.log")
    Write-Success " provide-foundation installed"
}
catch {
...
Write-Host "`n$("provide-foundation development environment activated" | Write-Host -ForegroundColor Green)"
Write-Host "Virtual environment: $VenvDir"
Write-Host "Profile: $Profile"
Write-Host "`nUseful commands:"
Write-Host "  provide-foundation --help  # provide-foundation CLI"

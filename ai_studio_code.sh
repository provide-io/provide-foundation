...
# --- Operations ---
log_info "Applying fixes for flaky tests..."

log_update "Updating: src/provide/foundation/errors/decorators.py to cache logger in ResilientErrorHandler"
mkdir -p src/provide/foundation/errors/
cat <<'EOF' > src/provide/foundation/errors/decorators.py
...
class ResilientErrorHandler:
    ...
    def _get_logger(self) -> Any:
        """Get and cache the foundation logger."""
        if self._logger is None:
            from provide.foundation.hub.foundation import get_foundation_logger

            self._logger = get_foundation_logger()
        return self._logger
...
log_update "Updating: src/provide/foundation/utils/rate_limiting.py to make _refill_tokens synchronous"
mkdir -p src/provide/foundation/utils/
cat <<'EOF' > src/provide/foundation/utils/rate_limiting.py
from __future__ import annotations
...
class TokenBucketRateLimiter:
    ...
    async def is_allowed(self) -> bool:
        ...
        async with self._lock:
            self._refill_tokens()  # Refill before checking

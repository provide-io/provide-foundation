...
1. **Switch to MD5 for cache keys in `serialization/cache.py`**
...
1. **Lazy dict copy in `logger/processors/sanitization.py`**
...
1. **Replace hash-based cache keys with tuples in `serialization/cache.py`**
...
1. **Optimized EventBus.emit() in `hub/events.py`**
...
1. **Skip gc.get_objects() scan in `testmode/internal.py`**
...
1. **Cached `is_in_click_testing()` in `testmode/detection.py`**
...
1. **`structlog/dev.py` ConsoleRenderer** -- 300K + 50K allocations. External dependency.
1. **`inspect.getmodule()`** -- 22K allocations / 1.17 GB in config stress. Called during structlog's stack introspection, not directly from Foundation code.
1. **`json/encoder.py` iterencode** -- 320K allocations / 689 MB in serialization stress. CPython's JSON encoder.
...
1. **Replaced dynamic class creation in `hub/initialization.py`**
...
1. **Reviewed structlog processor chain** (no change)
